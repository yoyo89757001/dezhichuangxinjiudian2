...
public class FacePassUtil {
  ...
  //  private int TIMEOUT=30*1000;
    ...
    private  final String group_name2 = "facepasstestx2";
    ...
    private  void checkGroup(Activity activity, final Context context) {
        ...
        if (localGroups == null || localGroups.length == 0) {
            try {
               ...
               boolean b= mFacePassHandler.createLocalGroup(group_name2);
                Log.d("FacePassUtil", "创建组:" + a+b);
                FacePassAddFaceResult result= mFacePassHandler.addFace(BitmapFactory.decodeResource(context.getResources(), R.drawable.aa));
                if (result.result==0){
                    mFacePassHandler.bindGroup(group_name2,result.faceToken);
                }
            } catch (FacePassException e) {
                e.printStackTrace();
            }
        }else {
            Log.d("FacePassUtil", "组名:"+ Arrays.toString(localGroups));
        }
    }
